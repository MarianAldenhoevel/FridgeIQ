...
class SVGPath:
    def get_float(self):
        ...
        m = re.match(r"[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?", self.d)
        if (not m):
            raise ValueError("expecting number, got {}".format(self.d))

        # self.d = self.d[len(m.group(1)):].lstrip(" /t/n,")
        # return float(m.group(1))
        self.d = self.d[len(m.group(0)):].lstrip(" /t/n,")
        return float(m.group(0))

    def get_cmd(self, default):
        if (re.match("[lLmMzZqQ]", self.d[0])):
            retval = self.d[0]
            self.d = self.d[1:].lstrip(" /t/n,")
            return retval
    ...
    def parse_path_string(self, d, trans):
        ...
        while self.d:
            ...
            if (cmd == 'q'):
                # Skip two values, then treat like m.
                self.get_float()
                self.get_float()
                cmd = 'm'
            elif (cmd == 'Q'):
...
def get_mm_from_dimension(val):
    mo = re.match("([0-9/./-]+)(mm|cm|m|in|ft|pt)", val)
    if (not mo):
        raise ValueError("expecting number with mm,cm,m,in,pt, or ft dimension, got '{}'".format(val))
...
def cleanchallenge(filepath):
    paths = parse_svg_path(filepath)
    ...
    for path in paths:
        ...
        if len(path.polys) == 1:
            for poly in path.polys:
                if len(poly) == 4: 
                    shards.append(Polygon(poly))
                    #print(shape)              

    shape = cascaded_union(shards) 
    ...
    props = {
        ...
        'width': '100%',
        'height': '100%',
        'viewBox': '{}'.format(','.join(map(str, shape.bounds))),
    ...
    raw_svg_str = r'''
        ...
        <svg {attrs:s}>
        <g id="challenge" style="fill: #00ff00;"
        {data:s}
        </g>
        </svg>
    '''.format(attrs=attrs, data=shape.svg()).strip()

    #print(raw_svg_str)
    with open(filepath + ".clean.svg", 'w') as f:
        f.write(raw_svg_str)

cleanchallenge('C:/Users/Marian Aldenhövel/Desktop/FridgeIQ/src/collection/c2b39e7d79a1f2b15a3fdb9ce34ed98e-solution.svg')
